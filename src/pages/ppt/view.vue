...
<template>
  <div class="ppt-root">
    <PageHeader class="page-header" v-model:doc="PPTState.nowPPT" show-more-actions>
      <template #headerAction>
        <el-dropdown>
          <el-button>
            <i class="ri-slideshow-2-line" style="margin-right: 6px"></i>
            演示
          </el-button>

          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="() => playPPT('firstPage')">从头播放</el-dropdown-item>
              <el-dropdown-item @click="() => playPPT('nowPage')">从当前页播放</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      </template>
...
watch(
  ...
  () => {
    if (isLoad.value) {
      PPTIframeRef.value?.contentWindow.postMessage({ type: "online-editor", action: "setEditable", data: false }, "*")
